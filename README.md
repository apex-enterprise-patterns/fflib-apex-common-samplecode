...
- **CRUD/FLS and user mode**—The previous sample relied on fflib defaults (Selector: LEGACY data access with object-level CRUD assertion; UoW: SimpleDML / system-mode DML); the preference now is fflib user mode, with selectors passing `fflib_SObjectSelector.DataAccess.USER_MODE` through the `super(...)` constructor.
...
| **USER_MODE / FLS** | System mode (no USER_MODE enforcement) | Selectors use `super(false, fflib_SObjectSelector.DataAccess.USER_MODE)` (and `includeFieldSetFields` overload where present); InvoicingTargetsRegistry query uses `WITH USER_MODE`; UnitOfWork uses `UserModeDML()` |
